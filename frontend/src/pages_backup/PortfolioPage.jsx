            ...
            <Card key={project.id} className="overflow-hidden hover:shadow-xl transition-shadow group border-none bg-white">
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <Badge className="bg-sport-blue text-white">{project.category}</Badge>
                  <div className="flex items-center gap-1 text-sm text-gray-500">
                    <Calendar size={16} />
                    <span>{project.year}</span>
                  </div>
                </div>
                <h3 className="text-2xl font-light mb-3 text-dark">{project.teamName}</h3>
                <p className="text-gray-600 mb-4 leading-relaxed">{project.description}</p>
                
                <div className="border-t border-gray-200 pt-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
                    <Users size={16} className="text-sport-blue" />
                    Выполненные работы:
                  </h4>
